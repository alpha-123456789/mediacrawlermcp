...
class ToutiaoArticle(Base):
    __tablename__ = 'toutiao_article'
    id = Column(Integer, primary_key=True, comment='主键ID')
    article_id = Column(String(64), nullable=False, index=True, unique=True, comment='文章ID')
    title = Column(Text, comment='文章标题')
    content = Column(Text, comment='文章纯文本内容')
    abstract = Column(Text, comment='文章摘要')
    source = Column(Text, comment='文章来源')
    url = Column(Text, comment='文章URL')
    user_id = Column(String(64), index=True, comment='用户ID')
    nickname = Column(Text, comment='作者/用户昵称')
    avatar_url = Column(Text, comment='用户头像')
    has_video = Column(Integer, default=0, comment='是否有视频 0-否 1-是')
    has_image = Column(Integer, default=0, comment='是否有图片 0-否 1-是')
    image_list = Column(Text, comment='图片列表（JSON格式）')
    video_duration = Column(Integer, default=0, comment='视频时长（秒）')
    read_count = Column(Integer, default=0, comment='阅读数')
    like_count = Column(Integer, default=0, comment='点赞数')
    digg_count = Column(Integer, default=0, comment='点赞数（别名）')
    bury_count = Column(Integer, default=0, comment='踩数')
    repin_count = Column(Integer, default=0, comment='转发数')
    comment_count = Column(Integer, default=0, comment='评论数')
    share_count = Column(Integer, default=0, comment='分享数')
    ip_location = Column(Text, comment='IP地址位置')
    publish_time = Column(BigInteger, index=True, comment='发布时间戳')
    create_time = Column(String(32), comment='文章创建时间（从内容中提取，如：2025-03-15 18:00）')
    media_id = Column(String(64), comment='媒体ID')
    media_name = Column(Text, comment='媒体名称')
    source_keyword = Column(Text, default='', comment='来源关键词')
    detail_fetched = Column(Integer, default=0, comment='是否获取详情 0-否 1-是')
    add_ts = Column(BigInteger, comment='添加时间戳')
    last_modify_ts = Column(BigInteger, comment='最后修改时间戳')


class ToutiaoComment(Base):
    __tablename__ = 'toutiao_comment'
    id = Column(Integer, primary_key=True, comment='主键ID')
    comment_id = Column(String(64), nullable=False, index=True, unique=True, comment='评论ID')
    article_id = Column(String(64), index=True, comment='文章ID')
    content = Column(Text, comment='评论内容')
    create_time = Column(BigInteger, index=True, comment='创建时间戳')
    like_count = Column(Integer, default=0, comment='点赞数')
    reply_count = Column(Integer, default=0, comment='回复数')
    is_sub_comment = Column(Integer, default=0, comment='是否子评论 0-否 1-是')
    user_id = Column(String(64), index=True, comment='用户ID')
    nickname = Column(Text, comment='用户昵称')
    avatar = Column(Text, comment='用户头像')
    parent_comment_id = Column(String(64), default='', comment='父评论ID')
    reply_to_user_id = Column(String(64), comment='回复目标用户ID')
    reply_to_nickname = Column(Text, comment='回复目标用户昵称')
    add_ts = Column(BigInteger, comment='添加时间戳')
    last_modify_ts = Column(BigInteger, comment='最后修改时间戳')


class ToutiaoCreator(Base):
    __tablename__ = 'toutiao_creator'
    id = Column(Integer, primary_key=True, comment='主键ID')
    creator_id = Column(String(64), nullable=False, index=True, unique=True, comment='创作者ID')
    nickname = Column(Text, comment='用户昵称')
    avatar = Column(Text, comment='用户头像')
